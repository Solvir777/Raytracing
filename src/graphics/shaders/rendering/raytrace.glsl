...
layout(rg8ui, set = 0, binding = 1) readonly uniform uimage3D voxeldata;
...
vec3 raycast() {
    ...
    if(imageLoad(voxeldata, rem_euclid_ivec3(ivec3(floor(ro)), voxel_data_size)).r > 0) {
    ...
    for(ivec3 offset = ivec3(0); all(lessThan(offset + ivec3(3)/*todo*/, ivec3(voxel_data_size / 2 - 1) + to_chunk_middle * oct_rd11));) {
        ...
        uint blocktype = imageLoad(voxeldata, rem_euclid_ivec3(pos, voxel_data_size)).r;
        if(blocktype > 0) {
